...
3. Use the interface to initiate an audio ingestion sequence. The frontend will capture the audio and send it to the Go backend.
4. The Go backend processes the audio, interfaces with the Gemini API, and returns the translated Hindi text, which is then displayed on the UI terminal.
